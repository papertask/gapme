...
if ( ! function_exists( 'ABdevFW_simple_breadcrumb' ) ){
	function ABdevFW_simple_breadcrumb(){
		...
		$text['events']= esc_html__('Events', 'ABdev_aeron');
		$text['events_list']= esc_html__('List', 'ABdev_aeron');
		...
		$link = '<a href="%s">%s</a>';
			...
			// 	echo $before . $text['events'] . $after . $delimiter . $before . $text['events_list'] . $after . $delimiter;
			...
			if ( get_query_var('paged') ) {
				...
				echo esc_html__('Page', 'ABdev_aeron') . ' ' . get_query_var('paged');
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
