...
function abdev_get_timeline_posts(){
	...
	if ($the_query -> have_posts()) :  while ($the_query -> have_posts()) : $the_query -> the_post();
		...
		$tag_out=array();
		$tag = get_the_tags();
		foreach ($tag as $tag_one) {
			$tag_out[] ='<a href="'.esc_url( get_tag_link( $tag_one->term_id ) ).'">' .$tag_one->name.'</a>';
		}
		$tag_out = implode(', ', $tag_out);

		$output .='<div class="timeline_postmeta">
				<h2><a href="'.esc_url(get_permalink()).'">'.get_the_title().'</a></h2>
				<i class="whhg-calendar"></i> <span class="date">'.get_the_date('M j, Y').'</span> 
				<i class="whhg-useralt"></i> '.get_the_author().'
				<i class="whhg-tags"></i> '.$tag_out.'
			</div>
